...
func CreateApi(api model.SysApi) (err error) {
	if !errors.Is(global.GqaDb.Where("path = ? AND method = ?", api.Path, api.Method).First(&model.SysApi{}).Error, gorm.ErrRecordNotFound) {
		return errors.New("存在相同api")
	}
	return global.GqaDb.Create(&api).Error
}
...
func DeleteApi(api model.SysApi) (err error) {
	err = global.GqaDb.Delete(api).Error
...
func GetAllApis() (err error, apis []model.SysApi) {
	err = global.GqaDb.Find(&apis).Error
	return
}
...
func GetApiById(id float64) (err error, api model.SysApi) {
	err = global.GqaDb.Where("id = ?", id).First(&api).Error
	return
}
...
func UpdateApi(api model.SysApi) (err error) {
	...
	err = global.GqaDb.Where("id = ?", api.ID).First(&oldA).Error
	...
	if err != nil {
		...
		if err != nil {
			...
			err = global.GqaDb.Save(&api).Error
		}
	}
	return err
}
