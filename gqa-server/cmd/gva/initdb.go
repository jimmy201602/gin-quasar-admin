...
var initdbCmd = &cobra.Command{
	Use:   "initdb",
	Short: "gin-quasar-admin初始化数据",
	Long: `gin-quasar-admin初始化数据适配数据库情况: 
1. mysql完美适配,
...
4. sqlserver未适配`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		global.GqaViper = core.Viper(path)
		global.GqaLog = core.Zap()           // 初始化zap日志库
		db := initialize.GormMysql()
		switch global.GqaConfig.System.DbType {
		case "mysql":
