...
func Login(c *gin.Context) {
	...
	if store.Verify(L.CaptchaId, L.Captcha, true) {
		...
		if err, user := service.Login(U); err != nil {
			global.GqaLog.Error("登陆失败! 用户名不存在或者密码错误", zap.Any("err", err))
...
func tokenNext(c *gin.Context, user model.SysUser) {
	j := &middleware.JWT{SigningKey: []byte(global.GqaConfig.JWT.SigningKey)} // 唯一签名
	...
	if err != nil {
		global.GqaLog.Error("获取token失败", zap.Any("err", err))
	...
	if err, jwtStr := service.GetRedisJWT(user.Username); err == redis.Nil {
		if err := service.SetRedisJWT(token, user.Username); err != nil {
			global.GqaLog.Error("设置登录状态失败", zap.Any("err", err))
...
func Register(c *gin.Context) {
	...
	if err != nil {
		global.GqaLog.Error("注册失败", zap.Any("err", err))
...
func ChangePassword(c *gin.Context) {
	...
	if err, _ := service.ChangePassword(U, user.NewPassword); err != nil {
		global.GqaLog.Error("修改失败", zap.Any("err", err))
...
func GetUserList(c *gin.Context) {
	...
	if err, list, total := service.GetUserInfoList(pageInfo); err != nil {
		global.GqaLog.Error("获取失败", zap.Any("err", err))
...
func DeleteUser(c *gin.Context) {
	...
	if err := service.DeleteUser(reqId.Id); err != nil {
		global.GqaLog.Error("删除失败!", zap.Any("err", err))
...
func SetUserInfo(c *gin.Context) {
	...
	if err, ReqUser := service.SetUserInfo(user); err != nil {
		global.GqaLog.Error("设置失败", zap.Any("err", err))
...
func getUserID(c *gin.Context) uint {
	if claims, exists := c.Get("claims"); !exists {
		global.GqaLog.Error("从Gin的Context中获取从jwt解析出来的用户ID失败, 请检查路由是否使用jwt中间件")
		return 0
	} else {
