...
func Routers() *gin.Engine {
	...
	Router.StaticFS(global.GqaConfig.Local.Path, http.Dir(global.GqaConfig.Local.Path)) // 为用户头像和文件提供静态地址
	// Router.Use(middleware.LoadTls())  // 打开就能玩https了
	global.GqaLog.Info("use middleware logger")
	// 跨域
	Router.Use(middleware.Cors())
	global.GqaLog.Info("use middleware cors")
	Router.GET("/swagger/*any", ginSwagger.WrapHandler(swaggerFiles.Handler))
	global.GqaLog.Info("register swagger handler")
	...
	global.GqaLog.Info("router register success")
	return Router
}
