...
func (g *GormLogger) Printf(message string, data ...interface{}) {
	switch len(data) {
	case 0:
		global.GqaLog.Info(message)
	case 1:
		global.GqaLog.Info("gorm", zap.Any("src", data[0]))
	case 2:
		global.GqaLog.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]))
	case 3:
		global.GqaLog.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]))
	case 4:
		global.GqaLog.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]), zap.Any("sql", data[3]))
	}
}
